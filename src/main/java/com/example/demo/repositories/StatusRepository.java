...
public interface StatusRepository extends JpaRepository<Status, Integer> {
    ...
    @Query(value = "Select s.*,t.*, e.* FROM tbl_m_status s JOIN tbl_m_employee e ON s.employee_id = e.id JOIN tbl_tr_travelexpenses t ON s.travelexpenses_id = t.id WHERE s.status = 'Request' AND s.employee_id in (select s.employee_id from tbl_m_status s group by s.employee_id having count(s.employee_id) = 1)", nativeQuery = true)
    public List<Status> findTravelExpenseStatus();

    @Query(value = "Select s.*, e.*, t.* FROM tbl_m_status s JOIN tbl_m_employee e ON s.employee_id = e.id JOIN tbl_tr_travelexpenses t ON s.travelexpenses_id = t.id WHERE s.status = 'Request' AND s.employee_id in (select s.employee_id from tbl_m_status s group by s.employee_id having count(s.employee_id) = 1)", nativeQuery = true)
    public List<TravelRequestDTO> findTravelExpenseStatusDTO(Integer id);

    @Query(value = "Select s.*,t.*, e.* FROM tbl_m_status s JOIN tbl_m_employee e ON s.employee_id = e.id JOIN tbl_tr_travelexpenses t ON s.travelexpenses_id = t.id WHERE s.status = 'Settlement Request' AND s.employee_id in (select s.employee_id from tbl_m_status s group by s.employee_id having count(s.employee_id)= 3)", nativeQuery = true)
    ...
    @Query(value = "Select s.*,t.*, e.* FROM tbl_m_status s JOIN tbl_m_employee e ON s.employee_id = e.id JOIN tbl_tr_travelexpenses t ON s.travelexpenses_id = t.id WHERE s.status = 'Approved Settlement Manager' AND s.employee_id in (select s.employee_id from tbl_m_status s group by s.employee_id having count(s.employee_id)= 4)", nativeQuery = true)
